...
if(isset($_POST["btn_changepass"])) {
    ...
    if ($new_password == $confirm_password) {
        ...
        $stmt = $conn->prepare("SELECT password FROM tbl_admin_user WHERE userid = ?");
        ...
        if ($new_password != $old_password) {
            ...
            if (preg_match('/^(?=.*[A-Z])(?=.*\d)(?=.*[^A-Za-z0-9\s])[\w\W]{8,}$/', $new_password)) {
                ...
                $update_query = "UPDATE tbl_admin_user SET password = '$new_password' WHERE userid = '".$_SESSION["userid"]."'";
